...
<br/>
In Part Three of this demonstration of using linear regression and some of its extensions, we seek to evaluate the **Life expectancy** model  (we created in Part Two) based on the data containing health, immunization, and economic and demographic information about 179 countries from 2000 to 2015. Namely, we will look at its predictive performance and we will also discuss the predictors that seem to have the greatest effect on predictions.
<br/>

## Evaluation of predictive performance (via cross-validation) 

<br/>
Let us discuss the correlated random effects model (CRE) we constructed in Part Two in terms of predictive performance. As a reminder, our model is 
<br/>
...
<br/> 
We used *lmer* for the fit instead of *plm*, because we plan to compare models using a likelihood ratio test in a bit and *plm* does not compute likelihood function since it uses estimates based on generalized least squares.

Let us start with an obligatory actual vs. predicted plot. We should note that prediction for a known individual is computed (using function *predict*) as $X\hat{\beta} + \hat{\tau} + \hat{\mu}$, where $X$ are our “main” predictors, $\hat{\beta}$ is the estimate of the coefficients for the “main” predictors, $\hat{\tau}$ are the estimates of the fixed time effects in the model, and $\hat{\mu}$ is the estimate of the individual random effect from the model (this estimate is known as *BLUP*, the best linear unbiased predictor). 
<br/>
...
<br/>
It seem that the model fits the data pretty well. Let us evaluate, how good this model would be for the predictions of life expectancy.  
...
This validation is slightly incorrect due to a  *data leakage* that is often easy to miss. The centered predictors for the train set were computed for each country using the whole dataset, i.e. using the test set data. The correct cross-validation should be computed as follows.
<br/>
...
<br/>
We observe that the results are only a tiny bit worse (this data leakage was not that severe).

Let us try the predictions for a new individual. Thus, we will now perform a cross-validation on the countries, i.e., using the rows of the dataset. I will perform 100 repetitions of the 10-fold cross-validation. We should emphasize that for a new individual, the prediction of the individual random effect is simply zero. We should note that the data leakage due to centered predictors are not the problem in this case, because these are computed for each countries, i.e., the train set and the test set is in this setup correctly separated.
<br/>
...
<br/>
Let us check the results. For simplicity's sake, we will compute a simple percentile-based confidence intervals (another popular more sophisticated option would be bias-corrected and accelerated (BCa) bootstrap that better handles skew and bias *B. Efron. Better bootstrap confidence intervals. Journal of the American statistical Association 82.397 (1987): 171-185.*)
<br/>
...
<br/>
Prediction intervals are interval estimates for a new observations. Provided that the model is correctly specified, the computation of the prediction interval is straightforward via the parametric bootstrap (we simply add the individual random effect and the idiosyncratic error to the prediction). 
<br/>
...
<br/>
Alternatively, we can also use residual cluster bootstrap to account for non-normal distribution and inter-cluster correlation of idiosyncratic errors.
<br/>
...
<br/>
We can fit a new model that models this heteroskedasticity by assuming a different idiosyncratic error variance per stratum developed and developing. We need to use a different package, *nlme*, which allows us to fit a mixed effects model with variance structure functions (see https://stat.ethz.ch/R-manual/R-devel/library/nlme/html/varClasses.html for the available options).
<br/>
...
<br/>
We see that, indeed, the estimated standard deviation of idiosyncratic errors is lower for developed countries. Thus, we can use this model instead to construct the prediction interval for France. We will use a wild bootstrap to generate new datasets and a residual bootstrap to simulate new observations for France. 
<br/>
...
<br/>
Let us return to our original model with fixed time effects. 
<br/>
...
<br/>
We will determine which predictors appeared to be significant in the model based on cluster-robust standard errors (CR2) and Satterthwaite DOF correction. We will also recompute confidence intervals based on these errors. Let us also remind ourselves of the ranges of values for the continuous variables. 
<br/>
...
<br/>
First, we will compute the confidence intervals for the predictions, varying one predictor while keeping the other fixed (other fixed effects are chosen as Turkey 2015) using a simple parametric bootstrap (we assume that the individual random effects are fixed).
<br/>
...
resid_sd <- as.data.frame(VarCorr(cre_model))$sdcor[2]

# Alcohol
Alcohol_seq <- seq(min(life_expectancy$Alcohol),max(life_expectancy$Alcohol),1)
Alcohol_obs <- life_expectancy[1,]
Alcohol_obs <- Alcohol_obs[rep(1, length(Alcohol_seq)), ]
Alcohol_obs$Alcohol  <- Alcohol_seq

# Hepatitis_B
Hepatitis_B_seq <- seq(min(life_expectancy$Hepatitis_B),max(life_expectancy$Hepatitis_B),5)
Hepatitis_B_obs <- life_expectancy[1,]
Hepatitis_B_obs <- Hepatitis_B_obs[rep(1, length(Hepatitis_B_seq)), ]
Hepatitis_B_obs$Hepatitis_B  <- Hepatitis_B_seq

# Measles
Measles_seq <- seq(min(life_expectancy$Measles),max(life_expectancy$Measles),5)
Measles_obs <- life_expectancy[1,]
Measles_obs <- Measles_obs[rep(1, length(Measles_seq)), ]
Measles_obs$Measles  <- Measles_seq


# BMI
BMI_seq <- seq(min(life_expectancy$BMI),max(life_expectancy$BMI),1)
BMI_obs <- life_expectancy[1,]
BMI_obs <- BMI_obs[rep(1, length(BMI_seq)), ]
BMI_obs$BMI  <- BMI_seq

# Polio
Polio_seq <- seq(min(life_expectancy$Polio),max(life_expectancy$Polio),5)
Polio_obs <- life_expectancy[1,]
Polio_obs <- Polio_obs[rep(1, length(Polio_seq)), ]
Polio_obs$Polio  <- Polio_seq

# Diphtheria
Diphtheria_seq <- seq(min(life_expectancy$Diphtheria),max(life_expectancy$Diphtheria),5)
Diphtheria_obs <- life_expectancy[1,]
Diphtheria_obs <- Diphtheria_obs[rep(1, length(Diphtheria_seq)), ]
Diphtheria_obs$Diphtheria  <- Diphtheria_seq

# Thin_5_9
Thin_5_9_seq <- seq(min(life_expectancy$Thin_5_9),max(life_expectancy$Thin_5_9),2)
Thin_5_9_obs <- life_expectancy[1,]
Thin_5_9_obs <- Thin_5_9_obs[rep(1, length(Thin_5_9_seq)), ]
Thin_5_9_obs$Thin_5_9  <- Thin_5_9_seq


# HIV
HIV_seq <- seq(min(life_expectancy$HIV),max(life_expectancy$HIV),1)
HIV_obs <- life_expectancy[1,]
HIV_obs <- Thin_5_9_obs[rep(1, length(HIV_seq)), ]
HIV_obs$HIV  <- HIV_seq

# Thin_10_19
Thin_10_19_seq <- seq(min(life_expectancy$Thin_10_19),max(life_expectancy$Thin_10_19),2)
Thin_10_19_obs <- life_expectancy[1,]
Thin_10_19_obs <- Thin_10_19_obs[rep(1, length(Thin_10_19_seq)), ]
Thin_10_19_obs$Thin_10_19  <- Thin_10_19_seq

# Schooling
Schooling_seq <- seq(min(life_expectancy$Schooling),max(life_expectancy$Schooling),1)
Schooling_obs <- life_expectancy[1,]
Schooling_obs <- Schooling_obs[rep(1, length(Schooling_seq)), ]
Schooling_obs$Schooling  <- Schooling_seq

# GDP_log
GDP_log_seq <- seq(min(life_expectancy$GDP_log),max(life_expectancy$GDP_log),0.5)
GDP_log_obs <- life_expectancy[1,]
GDP_log_obs <- GDP_log_obs[rep(1, length(GDP_log_seq)), ]
GDP_log_obs$GDP_log  <- GDP_log_seq

# Pop_log
Pop_log_seq <- seq(min(life_expectancy$Pop_log),max(life_expectancy$Pop_log),0.5)
Pop_log_obs <- life_expectancy[1,]
Pop_log_obs <- Pop_log_obs[rep(1, length(Pop_log_seq)), ]
Pop_log_obs$Pop_log  <- Pop_log_seq

# I_deaths
I_deaths_seq <- seq(min(life_expectancy$I_deaths),max(life_expectancy$I_deaths),0.2)
I_deaths_obs <- life_expectancy[1,]
I_deaths_obs <- I_deaths_obs[rep(1, length(I_deaths_seq)), ]
I_deaths_obs$I_deaths <- I_deaths_seq

# U5_deaths
U5_deaths_seq <- seq(min(life_expectancy$U5_deaths),max(life_expectancy$U5_deaths),0.2)
U5_deaths_obs <- life_expectancy[1,]
U5_deaths_obs <- U5_deaths_obs[rep(1, length(U5_deaths_seq)), ]
U5_deaths_obs$U5_deaths  <- U5_deaths_seq


Alcohol_pred <- matrix(0,1000,length(Alcohol_seq))
Hepatitis_B_pred <- matrix(0,1000,length(Hepatitis_B_seq))
Measles_pred <- matrix(0,1000,length(Measles_seq))
BMI_pred <- matrix(0,1000,length(BMI_seq))
Polio_pred <- matrix(0,1000,length(Polio_seq))
Diphtheria_pred <- matrix(0,1000,length(Diphtheria_seq))
HIV_pred <- matrix(0,1000,length(HIV_seq))
Thin_5_9_pred <- matrix(0,1000,length(Thin_5_9_seq))
Thin_10_19_pred <- matrix(0,1000,length(Thin_10_19_seq))
Schooling_pred <- matrix(0,1000,length(Schooling_seq))
GDP_log_pred <- matrix(0,1000,length(GDP_log_seq))
Pop_log_pred <- matrix(0,1000,length(Pop_log_seq))
I_deaths_pred <- matrix(0,1000,length(I_deaths_seq))
U5_deaths_pred <- matrix(0,1000,length(U5_deaths_seq))

for(i in 1:1000){
y <- predict(cre_model) + rnorm(dim(life_expectancy)[1],0,resid_sd)
bmod <- refit(cre_model, y)
Alcohol_pred[i,] <- predict(bmod,Alcohol_obs)
Hepatitis_B_pred[i,] <- predict(bmod,Hepatitis_B_obs)
Measles_pred[i,] <- predict(bmod,Measles_obs)
BMI_pred[i,] <- predict(bmod,BMI_obs)
Polio_pred[i,] <- predict(bmod,Polio_obs)
Diphtheria_pred[i,] <- predict(bmod,Diphtheria_obs)
HIV_pred[i,] <- predict(bmod,HIV_obs)
Thin_5_9_pred[i,] <- predict(bmod,Thin_5_9_obs)
Thin_10_19_pred[i,] <- predict(bmod,Thin_10_19_obs)
Schooling_pred[i,] <- predict(bmod,Schooling_obs)
Pop_log_pred[i,] <- predict(bmod,Pop_log_obs)
GDP_log_pred[i,] <- predict(bmod,GDP_log_obs)
I_deaths_pred[i,] <- predict(bmod,I_deaths_obs)
U5_deaths_pred[i,] <- predict(bmod,U5_deaths_obs)
}


Alcohol_CI <- cbind(predict(cre_model,Alcohol_obs), t(apply(Alcohol_pred,2,function(x) quantile(x[!is.na(x)],c(0.025,0.975)))))
colnames(Alcohol_CI) <- c('fit','lwr','upr')

Hepatitis_B_CI <- cbind(predict(cre_model,Hepatitis_B_obs), t(apply(Hepatitis_B_pred,2,function(x) quantile(x[!is.na(x)],c(0.025,0.975)))))
colnames(Hepatitis_B_CI) <- c('fit','lwr','upr')

Measles_CI <- cbind(predict(cre_model,Measles_obs), t(apply(Measles_pred,2,function(x) quantile(x[!is.na(x)],c(0.025,0.975)))))
colnames(Measles_CI) <- c('fit','lwr','upr')

BMI_CI <- cbind(predict(cre_model,BMI_obs), t(apply(BMI_pred,2,function(x) quantile(x[!is.na(x)],c(0.025,0.975)))))
colnames(BMI_CI) <- c('fit','lwr','upr')

Polio_CI <- cbind(predict(cre_model,Polio_obs), t(apply(Polio_pred,2,function(x) quantile(x[!is.na(x)],c(0.025,0.975)))))
colnames(Polio_CI) <- c('fit','lwr','upr')

Diphtheria_CI <- cbind(predict(cre_model,Diphtheria_obs), t(apply(Diphtheria_pred,2,function(x) quantile(x[!is.na(x)],c(0.025,0.975)))))
colnames(Diphtheria_CI) <- c('fit','lwr','upr')

HIV_CI <- cbind(predict(cre_model,HIV_obs), t(apply(HIV_pred,2,function(x) quantile(x[!is.na(x)],c(0.025,0.975)))))
colnames(HIV_CI) <- c('fit','lwr','upr')

Thin_10_19_CI <- cbind(predict(cre_model,Thin_10_19_obs), t(apply(Thin_10_19_pred,2,function(x) quantile(x[!is.na(x)],c(0.025,0.975)))))
colnames(Thin_10_19_CI) <- c('fit','lwr','upr')

Thin_5_9_CI <- cbind(predict(cre_model,Thin_5_9_obs), t(apply(Thin_5_9_pred,2,function(x) quantile(x[!is.na(x)],c(0.025,0.975)))))
colnames(Thin_5_9_CI) <- c('fit','lwr','upr')

Schooling_CI <- cbind(predict(cre_model,Schooling_obs), t(apply(Schooling_pred,2,function(x) quantile(x[!is.na(x)],c(0.025,0.975)))))
colnames(Schooling_CI) <- c('fit','lwr','upr')

GDP_log_CI <- cbind(predict(cre_model,GDP_log_obs), t(apply(GDP_log_pred,2,function(x) quantile(x[!is.na(x)],c(0.025,0.975)))))
colnames(GDP_log_CI) <- c('fit','lwr','upr')

Pop_log_CI <- cbind(predict(cre_model,Pop_log_obs), t(apply(Pop_log_pred,2,function(x) quantile(x[!is.na(x)],c(0.025,0.975)))))
colnames(Pop_log_CI) <- c('fit','lwr','upr')

I_deaths_CI <- cbind(predict(cre_model,I_deaths_obs), t(apply(I_deaths_pred,2,function(x) quantile(x[!is.na(x)],c(0.025,0.975)))))
colnames(I_deaths_CI) <- c('fit','lwr','upr')

U5_deaths_CI <- cbind(predict(cre_model,U5_deaths_obs), t(apply(U5_deaths_pred,2,function(x) quantile(x[!is.na(x)],c(0.025,0.975)))))
colnames(U5_deaths_CI) <- c('fit','lwr','upr')


# Alcohol and Hepatitis_B
plot1 <- ggplot(aes(x=Alcohol_seq, y=fit, ymin=lwr, ymax=upr), data=Alcohol_CI) + geom_point() + geom_ribbon(linetype=2, alpha=0.1) + labs(x="Alcohol", y="Prediction (Turkey 2015) w/ 95% CI") + theme_bw()
plot2 <- ggplot(aes(x=Hepatitis_B_seq, y=fit, ymin=lwr, ymax=upr), data=Hepatitis_B_CI) + geom_point() + geom_ribbon(linetype=2, alpha=0.1) + labs(x="Hepatitis_B", y="Prediction (Turkey 2015) w/ 95% CI") + theme_bw()
grid.arrange(plot1, plot2, ncol=2)

# Measles and Polio
plot1 <- ggplot(aes(x=Measles_seq, y=fit, ymin=lwr, ymax=upr), data=Measles_CI) + geom_point() + geom_ribbon(linetype=2, alpha=0.1) + labs(x="Measles", y="Prediction (Turkey 2015) w/ 95% CI") + theme_bw()
plot2 <- ggplot(aes(x=Polio_seq, y=fit, ymin=lwr, ymax=upr), data=Polio_CI) + geom_point() + geom_ribbon(linetype=2, alpha=0.1) + labs(x="Polio", y="Prediction (Turkey 2015) w/ 95% CI") + theme_bw()
grid.arrange(plot1, plot2, ncol=2)

# Diphtheria and Pop_log
plot1 <- ggplot(aes(x=Diphtheria_seq, y=fit, ymin=lwr, ymax=upr), data=Diphtheria_CI) + geom_point() + geom_ribbon(linetype=2, alpha=0.1) + labs(x="Diphtheria", y="Prediction (Turkey 2015) w/ 95% CI") + theme_bw()
plot2 <- ggplot(aes(x=Pop_log_seq, y=fit, ymin=lwr, ymax=upr), data=Pop_log_CI) + geom_point() + geom_ribbon(linetype=2, alpha=0.1) + labs(x="Pop_log", y="Prediction (Turkey 2015) w/ 95% CI") + theme_bw()


# Thin_10_19 and Thin_5_9
plot1 <- ggplot(aes(x=Thin_10_19_seq, y=fit, ymin=lwr, ymax=upr), data=Thin_10_19_CI) + geom_point() + geom_ribbon(linetype=2, alpha=0.1) + labs(x="Thin_10_19", y="Prediction (Turkey 2015) w/ 95% CI") + theme_bw()
plot2 <- ggplot(aes(x=Thin_5_9_seq, y=fit, ymin=lwr, ymax=upr), data=Thin_5_9_CI) + geom_point() + geom_ribbon(linetype=2, alpha=0.1) + labs(x="Thin_5_9", y="Prediction (Turkey 2015) w/ 95% CI") + theme_bw()
grid.arrange(plot1, plot2, ncol=2)

# Schooling and GDP_log
plot1 <- ggplot(aes(x=Schooling_seq, y=fit, ymin=lwr, ymax=upr), data=Schooling_CI) + geom_point() + geom_ribbon(linetype=2, alpha=0.1) + labs(x="Schooling", y="Prediction (Turkey 2015) w/ 95% CI") + theme_bw()
plot2 <- ggplot(aes(x=GDP_log_seq, y=fit, ymin=lwr, ymax=upr), data=GDP_log_CI) + geom_point() + geom_ribbon(linetype=2, alpha=0.1) + labs(x="GDP_log", y="Prediction (Turkey 2015) w/ 95% CI") + theme_bw()
grid.arrange(plot1, plot2, ncol=2)

# BMI
ggplot(aes(x=BMI_seq, y=fit, ymin=lwr, ymax=upr), data=BMI_CI) + geom_point() + geom_ribbon(linetype=2, alpha=0.1) + labs(x="BMI", y="Prediction (Turkey 2015) w/ 95% CI") + theme_bw()

# HIV
ggplot(aes(x=HIV_seq, y=fit, ymin=lwr, ymax=upr), data=HIV_CI) + geom_point() + geom_ribbon(linetype=2, alpha=0.1) + labs(x="HIV", y="Prediction (Turkey 2015) w/ 95% CI") + theme_bw()

# I_deaths and U5_deaths
ggplot(aes(x=I_deaths_seq, y=fit, ymin=lwr, ymax=upr), data=I_deaths_CI) + geom_point() + geom_ribbon(linetype=2, alpha=0.1) + labs(x="I_deaths", y="Prediction (Turkey 2015) w/ 95% CI") + theme_bw()

ggplot(aes(x=U5_deaths_seq, y=fit, ymin=lwr, ymax=upr), data=U5_deaths_CI) + geom_point() + geom_ribbon(linetype=2, alpha=0.1) + labs(x="U5_deaths", y="Prediction (Turkey 2015) w/ 95% CI") + theme_bw()
```

<br/>
We see that many predictors seem to have little absolute effect: **Alcohol**, **Hepatitis_B**, **Measles**, **Polio**, **Diphtheria**, **Pop_log**, **Thin_10_19**, **Thin_5_9**, and **Schooling**.
Let us have a closer look at the rest. 

Let us start with the time-invariant predictors that we were able to estimate thanks to the CRE model: **Economy** and **Region**-specific factors. As far as **Economy** is concerned, it is a highly significant predictor in the model. It seems that economically developed countries tend to have higher life expectancy than developing countries, even after adjusting for other covariates in the model. We can investigate this more formally post hoc using *lsmeans* (lsmeans computes estimated marginal means for a given factor, see https://cran.r-project.org/web/packages/emmeans/vignettes/basics.html for more details).
<br/>
...
<br/>
Next, we focus on the **Region**-specific factors. We can test formally that this factor **Region** is significant in the model. 
<br/>
...
<br/>
However, the differences between the estimated marginal means for regions seem overall pretty close. 
<br/>
...
<br/>
So overall, the differences between regions after adjusting for other covariates are not that noticeable.

Let us move to the time-varying  predictors in our model. We start with **I_deaths** and **U5_deaths**, i.e., infant mortality/deaths of children under five years old per 1000 population. This effect appears strongly significant in the model. This is expected since the incidence of such early deaths necessarily drives the life expectancy down. We could observe this effect quite clearly from the data (the red curve is a LOESS fit of the data: span = 0.5, degree = 2)
<br/>

```{r, fig.align = 'center', echo=FALSE}
a <- loess(Life_expectancy~I_deaths,data = life_expectancy, span = 0.5, degree = 2)
b <- cbind(predict(a),life_expectancy$I_deaths)[order(life_expectancy$I_deaths),]
plot(life_expectancy$I_deaths,life_expectancy$Life_expectancy,xlab = 'I_deaths',
ylab = 'Life expectancy')
lines(b[,2],b[,1], col = "red", lwd = 2) 

a <- loess(Life_expectancy~U5_deaths,data = life_expectancy, span = 0.5, degree = 2)
b <- cbind(predict(a),life_expectancy$U5_deaths)[order(life_expectancy$U5_deaths),]
plot(life_expectancy$U5_deaths,life_expectancy$Life_expectancy,xlab = 'U5_deaths',
ylab = 'Life expectancy')
lines(b[,2],b[,1], col = "red", lwd = 2) 
```

<br/>
Another predictor worth mentioning is **GDP_log**. Now, this predictor is not significant (using a commonly used p-value cut-off 0.05) in the model. The confidence interval for the estimated effect is a bit too large. However, its point estimate is positive, as one would expect; more economically developed countries tend to have higher life expectancy.
<br/>

```{r, fig.align = 'center', echo=FALSE}
a <- loess(Life_expectancy~GDP_log,data = life_expectancy, span = 0.5, degree = 2)
b <- cbind(predict(a),life_expectancy$GDP_log)[order(life_expectancy$GDP_log),]
plot(life_expectancy$GDP_log,life_expectancy$Life_expectancy,xlab = 'GDP_log',
ylab = 'Life expectancy')
lines(b[,2],b[,1], col = "red", lwd = 2) 
```

<br/>
Another highly significant predictor in the model is the number of **HIV** incidents. The importance of this effect is again to be expected. For example, the UNAIDS report *THE URGENCY OF NOW AIDS AT A CROSSROADS* shows that from successes in the treatment of HIV, life expectancy in Africa increased from 56 to 61 between 2010 and 2024. Again, if we visualize the data, the effect of **HIV** is also quite noticeable.
<br/>
...
<br/>
The last predictor we need to  mention is the average **BMI** of the adult population. This one is a bit trickier to interpret. If we simply visualize the data, we could argue that **Life_expectancy** actually increases slightly with **BMI**. 
<br/>
