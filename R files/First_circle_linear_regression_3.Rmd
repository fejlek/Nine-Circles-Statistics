...
# Inf5_m
Inf5_m_seq <- seq(min(life_expectancy$Inf5_m),max(life_expectancy$Inf5_m),0.2)
Inf5_m_obs <- life_expectancy[1,]
Inf5_m_obs <- Inf5_m_obs[rep(1, length(Inf5_m_seq)), ]
Inf5_m_obs$Inf5_m  <- Inf5_m_seq
...
Inf5_m_pred <- matrix(0,1000,length(Inf5_m_seq))
...
Inf5_m_pred[i,] <- predict(bmod,Inf5_m_obs)
}
...
Inf5_m_CI <- cbind(predict(cre_model,Inf5_m_obs), t(apply(Inf5_m_pred,2,function(x) quantile(x[!is.na(x)],c(0.025,0.975)))))
colnames(Inf5_m_CI) <- c('fit','lwr','upr')
...
# Inf5_m
ggplot(aes(x=Inf5_m_seq, y=fit, ymin=lwr, ymax=upr), data=Inf5_m_CI) + geom_point() + geom_ribbon(linetype=2, alpha=0.1) + labs(x="Inf5_m", y="Prediction (Turkey 2015) w/ 95% CI") + theme_bw()
