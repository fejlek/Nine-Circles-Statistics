...
quan_glm3 <- rbind(quan_glm2,
                   sqrt(quantile((predict(model_glm_gammaloglink, type = 'response') -
                                    House_Rent$Rent)^2,c(0.05,0.25,0.5,0.75,0.95))),
                   sqrt(quantile((predict(model_glm_gammaloglink_red, House_Rent, type = 'response') -
                                    House_Rent$Rent)^2,c(0.05,0.25,0.5,0.75,0.95)))
)
...
<br/>
We expect that the variance function will be far too strong. 
<br/>
...
quan_glm4 <- rbind(quan_glm3,
                   sqrt(quantile((predict(model_glm_igaussloglink, type = 'response') -
                                    House_Rent$Rent)^2,c(0.05,0.25,0.5,0.75,0.95))))
...
<br/>
Let us plot the effects of predictors using the quantile models.
<br/>
...
```{r, message=FALSE, warning=FALSE,echo=FALSE}
# Floor
plot1 <- plot_model(model_med, type = "pred", terms = c('Floor', 'City [Mumbai]'))
plot2 <- plot_model(model_05, type = "pred", terms = c('Floor', 'City [Mumbai]'))
plot3 <- plot_model(model_25, type = "pred", terms = c('Floor', 'City [Mumbai]')) 
plot4 <- plot_model(model_75, type = "pred", terms = c('Floor', 'City [Mumbai]')) 
plot5 <- plot_model(model_95, type = "pred", terms = c('Floor', 'City [Mumbai]'))
...
ggplot() +
  ...
  geom_line(data = plot4$data,aes(x = x, y = predicted, color = "0.75 quantile")) +
  geom_line(data = plot5$data,aes(x = x, y = predicted, color = "0.95 quantile")) + 
geom_ribbon(data = plot5$data,aes(ymin = conf.low, ymax = conf.high, x = x, fill = "0.95 quantile"),alpha = 0.1, linewidth = 0, show.legend = FALSE) + labs(title="Mumbai", x="Floor", y="Predicted Log Rent")

# Max_Floor 
plot1 <- plot_model(model_med, type = "pred", terms = c('Max_Floor', 'City [Bangalore]'))
plot2 <- plot_model(model_05, type = "pred", terms = c('Max_Floor', 'City [Bangalore]'))
plot3 <- plot_model(model_25, type = "pred", terms = c('Max_Floor', 'City [Bangalore]')) 
plot4 <- plot_model(model_75, type = "pred", terms = c('Max_Floor', 'City [Bangalore]')) 
plot5 <- plot_model(model_95, type = "pred", terms = c('Max_Floor', 'City [Bangalore]'))
...
ggplot() +
  ...
  geom_line(data = plot4$data,aes(x = x, y = predicted, color = "0.75 quantile")) +
  geom_line(data = plot5$data,aes(x = x, y = predicted, color = "0.95 quantile")) + 
geom_ribbon(data = plot5$data,aes(ymin = conf.low, ymax = conf.high, x = x, fill = "0.95 quantile"),alpha = 0.1, linewidth = 0, show.legend = FALSE) + labs(title="Bangalore", x="Max_Floor", y="Predicted Log Rent")
...
geom_ribbon(data = plot5$data,aes(ymin = conf.low, ymax = conf.high, x = x, fill = "0.95 quantile"),alpha = 0.1, linewidth = 0, show.legend = FALSE) + labs(title="Kolkata", x="City", y="Predicted Log Rent") +
...
Additionally, the regression lines for the quantiles are not parallel in some plots. This so-called location shift test [6] can be tested more formally via the Wald test.
<br/>
