...
<br/>
In this project, we will model an ordinal outcome, i.e., categorical data with a natural order. We will mainly cover the ordered logit model (proportional odds logistic regression). We will also briefly cover other ordinal outcomes models: the continuation ratio model, the adjacent categories models, and the multinomial logit model.
...
<br/>
To model the ordinal response (in this case, the final grade), we will first use the ordered logit model. Ordered logit models cumulative distribution functions $P[Y \leq k ] = \mathrm{ilogit} (\theta_k - X\beta)$ for ordinal response $k = 1,2,3, \ldots$. The model is called proportional since the parameter $\beta$ does not depend on class $k$.
...
The last thing to check is the presence of influential observations. The package *VGAM* includes some basic functions for such diagnostics. Let us first plot the hat values (leverages) for each observation.
<br/>
...
<br/>
We observe that some observations might be overly influential. Let us check, whether deleting observations based on several leverage thresholds change the fit significantly.
<br/>
...
<br/>
We do not observe major changes in the fits, thus, there we found no reason to consider deleting observations.
<br/>
